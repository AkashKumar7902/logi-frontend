...
function DriverRegister() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setFormError(getApiErrorMessage(err, "Unable to register"));
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <Card className="w-full max-w-md">
        <CardBody className="p-7">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-semibold text-ink-900 dark:text-ink-50">Drive with Logi</h2>
            <p className="mt-1 text-sm text-ink-500 dark:text-ink-400">
              Earn on your schedule — flexible, fast, reliable
            </p>
          </div>

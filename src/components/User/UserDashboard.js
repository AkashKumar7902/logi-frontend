...
const UserDashboard = () => {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      const bookingRequest = {
        ...
        dropoff_location: {
          type: "Point",
          coordinates: [parseFloat(dropoff.longitude), parseFloat(dropoff.latitude)],
        },
  ...
  const fetchRoute = async (start, end) => {
    ...
    const handlePickupSelect = (location) => {
      setFormData({ ...formData, pickupLongitude: location?.longitude, pickupLatitude: location?.latitude });
      setPickup(location);
    };
  
    const handleDropoffSelect = (location) => {
      setFormData({ ...formData, dropoffLongitude: location?.longitude, dropoffLatitude: location?.latitude });
      setDropoff(location);
